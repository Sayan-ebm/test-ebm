#========================================================
# SurvivalinterventionServer.R
#========================================================
#########################################################
#### Guard: auto-correct blank/invalid manual entries ###
#########################################################
guardNumericInput(input, session, "int_median_os", default = 50, min = 0, label = "Intervention Median OS (Months)")
guardNumericInput(input, session, "int_hr_os", default = 0.5, min = 0, label = "Intervention OS Hazard Ratio")
guardNumericInput(input, session, "int_median_pfs", default = 25, min = 0, label = "Intervention Median PFS (Months)")
guardNumericInput(input, session, "int_hr_pfs", default = 0.5, min = 0, label = "Intervention PFS Hazard Ratio")
guardNumericInput(input, session, "int_longterm_os_prob", default = 0.05, min = 0, max = 1, label = "Intervention OS Survival Probability")
guardNumericInput(input, session, "int_longterm_os_time", default = 60, min = 0, label = "Intervention OS Long-Term Time (Months)")
guardNumericInput(input, session, "int_longterm_pfs_prob", default = 0.02, min = 0, max = 1, label = "Intervention PFS Survival Probability")
guardNumericInput(input, session, "int_longterm_pfs_time", default = 60, min = 0, label = "Intervention PFS Long-Term Time (Months)")
...
#########################################################
#### RESET TO DEFAULT (INTERVENTION SURVIVAL DATA) ######
#########################################################
...
int_reset_flags <- reactiveValues(
  int_os_ipd  = FALSE,
  int_os_km   = FALSE,
  int_pfs_ipd = FALSE,
  int_pfs_km  = FALSE
...
int_os_ipd_file <- reactive({
  if (isTRUE(int_reset_flags$int_os_ipd)) return(NULL)
  input$int_os_ipd
})
int_os_km_survival_file <- reactive({
  if (isTRUE(int_reset_flags$int_os_km)) return(NULL)
  input$int_os_km_survival
})
int_os_km_risk_file <- reactive({
  if (isTRUE(int_reset_flags$int_os_km)) return(NULL)
  input$int_os_km_risk
})
int_pfs_ipd_file <- reactive({
  if (isTRUE(int_reset_flags$int_pfs_ipd)) return(NULL)
  input$int_pfs_ipd
})
int_pfs_km_survival_file <- reactive({
  if (isTRUE(int_reset_flags$int_pfs_km)) return(NULL)
  input$int_pfs_km_survival
})
int_pfs_km_risk_file <- reactive({
  if (isTRUE(int_reset_flags$int_pfs_km)) return(NULL)
  input$int_pfs_km_risk
})

# Clear the flag as soon as the user picks a genuinely new file
observeEvent(input$int_os_ipd, { int_reset_flags$int_os_ipd <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)
observeEvent(input$int_os_km_survival, { int_reset_flags$int_os_km <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)
observeEvent(input$int_os_km_risk, { int_reset_flags$int_os_km <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)
observeEvent(input$int_pfs_ipd, { int_reset_flags$int_pfs_ipd <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)
observeEvent(input$int_pfs_km_survival, { int_reset_flags$int_pfs_km <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)
observeEvent(input$int_pfs_km_risk, { int_reset_flags$int_pfs_km <- FALSE }, ignoreInit = TRUE, ignoreNULL = TRUE)

observeEvent(input$reset_int_os_ipd, {
  int_reset_flags$int_os_ipd <- TRUE
  shinyjs::reset("int_os_ipd")
  showNotification("Intervention OS IPD reset to default dataset.", type = "message")
})

observeEvent(input$reset_int_os_km, {
  int_reset_flags$int_os_km <- TRUE
  shinyjs::reset("int_os_km_survival")
  shinyjs::reset("int_os_km_risk")
  showNotification("Intervention OS digitized KM data reset to default dataset.", type = "message")
})

observeEvent(input$reset_int_pfs_ipd, {
  int_reset_flags$int_pfs_ipd <- TRUE
  shinyjs::reset("int_pfs_ipd")
  showNotification("Intervention PFS IPD reset to default dataset.", type = "message")
})

observeEvent(input$reset_int_pfs_km, {
  int_reset_flags$int_pfs_km <- TRUE
  shinyjs::reset("int_pfs_km_survival")
  shinyjs::reset("int_pfs_km_risk")
  showNotification("Intervention PFS digitized KM data reset to default dataset.", type = "message")
})
...
#########################################################
#### Download Intervention Median OS Predictions ########
#########################################################
output$download_int_median_os_predictions <- downloadHandler(
  filename = function() {
    paste0("Intervention_Median_OS_Predictions_", Sys.Date(), ".csv")
  },
  content = function(file) {
    pred <- median_os_curve()
    pred_export <- data.frame(
      Time_Months = pred$time,
      Survival_Probability = pred$survival
    )
    write.csv(pred_export, file, row.names = FALSE)
  }
)
...
#########################################################
#### Download Intervention Median PFS Predictions #######
#########################################################
output$download_int_median_pfs_predictions <- downloadHandler(
  filename = function() {
    paste0("Intervention_Median_PFS_Predictions_", Sys.Date(), ".csv")
  },
  content = function(file) {
    pred <- median_pfs_curve()
    pred_export <- data.frame(
      Time_Months = pred$time,
      Survival_Probability = pred$survival
    )
...
#########################################################
km_os_data <- reactive({
  req(int_os_km_survival_file())
  km <- read.csv(
    int_os_km_survival_file()$datapath
  )
  validate(
    need(
...
km_pfs_data <- reactive({
  req(int_pfs_km_survival_file())
  km <- read.csv(
    int_pfs_km_survival_file()$datapath
  )
  validate(
    need(
      all(c("time", "survival") %in% names(km)),
...
output$download_os_survival_example <- downloadHandler(
  filename = function() {
    "Example_Intervention_OS_Survival.csv"
  },
  content = function(file) {
    write.csv(default_os_survival_intervention, file, row.names = FALSE)
  }
)
...
output$download_os_risk_example <- downloadHandler(
  filename = function() {
    "Example_Intervention_OS_Risk.csv"
  },
  content = function(file) {
    write.csv(default_os_risk_intervention, file, row.names = FALSE)
  }
)
...
output$int_median_os_plot <- plotly::renderPlotly({
  ...
  # OPTIONAL KM CURVE
  #------------------------------------------------------
  if (!is.null(int_os_km_survival_file())) {
    km <- km_os_data()
    p <- p %>%
      plotly::add_lines(
        data = km,
...
output$int_median_pfs_plot <- plotly::renderPlotly({
  ...
  # OPTIONAL KM CURVE
  #------------------------------------------------------
  if (!is.null(int_pfs_km_survival_file())) {
    km <- km_pfs_data()
    p <- p %>%
      plotly::add_lines(
        data = km,
...
output$download_os_ipd_example <- downloadHandler(
  filename = function() {
    paste0("Intervention_OS_IPD_", Sys.Date(), ".csv")
  },
  content = function(file) {
    write.csv(default_os_ipd_intervention, file, row.names = FALSE)
  }
)
...
int_os_ipd <- reactive({
  ...
  #######################################################
  if(input$int_os_survival_type == "IPD"){
    if(is.null(int_os_ipd_file())){
      ipd <- default_os_ipd_intervention
    } else {
      ipd <- read_ipd_data(
        int_os_ipd_file()
      )
  ...
  #### DIGITIZED KM ROUTE ###############################
  #######################################################
  km_data <- if(is.null(int_os_km_survival_file())){
    default_os_survival_intervention
  } else {
    read.csv(
      int_os_km_survival_file()$datapath,
  ...
  risk_data <- if(is.null(int_os_km_risk_file())){
    default_os_risk_intervention
  } else {
    read.csv(
      int_os_km_risk_file()$datapath,
      stringsAsFactors = FALSE
    )
  }
  reconstruct_ipd(
...
output$int_os_ipd_source <- renderText({
  if(is.null(int_os_ipd_file())){
    "Currently using: Default Intervention OS IPD dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_os_ipd_file()$name
    )
  }
})

output$int_os_survival_source <- renderText({
  if(is.null(int_os_km_survival_file())){
    "Currently using: Default Intervention OS Survival dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_os_km_survival_file()$name
    )
  }
})

output$int_os_risk_source <- renderText({
  if(is.null(int_os_km_risk_file())){
    "Currently using: Default Intervention OS Risk dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_os_km_risk_file()$name
    )
  }
})
...
output$download_int_os_model_output <- downloadHandler(
  filename = function() {
    paste0("Intervention_OS_Model_Output_", Sys.Date(), ".xlsx")
  },
  content = function(file) {
    ...
    #### Parameters
    #################################################
    params <- extract_model_parameters(int_os_models())
    ...
    openxlsx::writeData(wb, "Predictions", int_os_predictions())
    #################################################
    #### Save
    #################################################
...
get_primary_os_model <- function() {
  selected <- selected_int_os_models()
  if(length(selected) > 0){
    return(selected[1])
  }
  int_os_fit_table()$Model[1]
}
...
output$download_longterm_os_example <- downloadHandler(
  ...
  content = function(file) {
    write.csv(longterm_os_int, file, row.names = FALSE)
  }
)
...
get_comp_os_model <- function() {
  selected <- selected_comp_os_models()
  if(length(selected) > 0){
    return(selected[1])
  }
  comp_os_fit_table()$Model[1]
}


#########################################################
#### Active Comparator OS Curve #########################
#########################################################
get_active_comp_os_curve <- reactive({
  cat("\n===== ENTERING get_active_comp_os_curve =====\n")
  if (input$comp_os_input_type == "Median OS") {
    cat("Using Median OS comparator\n")
    obj <- comp_os_object()
    req(obj)
    return(
      get_active_curve(
...
int_os_curve <- reactive({
  cat("\n====================\n")
  cat("ENTERING int_os_curve\n")
  cat("====================\n")
  cat("input$int_os_input_type = ")
  print(input$int_os_input_type)
  cat("input$int_hr_os = ")
  print(input$int_hr_os)
  #################################################
  #### Hazard Ratio ###############################
  #################################################
  if(input$int_os_input_type == "Hazard Ratio"){
    ...
    validate(
      need(
        ...
        paste(
          "Comparator type is",
          input$comp_os_input_type
        )
      )
    )
    comp_curve <- tryCatch(
      get_active_comp_os_curve(),
      error = function(e){
        cat("ERROR INSIDE get_active_comp_os_curve()\n")
        print(e)
        return(NULL)
      }
    )
  ...
  curve <- get_active_curve(
    survival_object = obj$curve,
    selected_model = get_primary_os_model()
  )
  curve
})

observe({
  req(get_active_comp_os_curve())
  cat("\n===== ACTIVE COMP OS CURVE =====\n")
  cat("Comparator type:\n")
  print(input$comp_os_input_type)
  cat("Model:\n")
  print(unique(get_active_comp_os_curve()$model))
  cat("Month 12:\n")
  print(
    get_active_comp_os_curve()[
      get_active_comp_os_curve()$time == 12,
    ]
  )
})
...
output$int_hr_os_plot <- plotly::renderPlotly({
  ...
  comp_curve <- get_active_comp_os_curve()
  int_curve <- int_os_curve()
...
observe({
  req(get_active_comp_os_curve())
  cat("\n=====================\n")
  cat("ACTIVE COMP OS CURVE\n")
  cat("=====================\n")
  print(
    unique(
      get_active_comp_os_curve()$model
    )
  )
  print(
    get_active_comp_os_curve()[
...
int_pfs_ipd <- reactive({
  ...
  if(input$int_pfs_survival_type == "IPD"){
    if(is.null(int_pfs_ipd_file())){
      default_pfs_ipd_intervention
    } else {
      read_ipd_data(int_pfs_ipd_file())
    }
  } else {
    km_data <- if(is.null(int_pfs_km_survival_file())){
      default_pfs_survival_intervention
    } else {
      read.csv(int_pfs_km_survival_file()$datapath)
    }
    risk_data <- if(is.null(int_pfs_km_risk_file())){
      default_pfs_risk_intervention
    } else {
      read.csv(int_pfs_km_risk_file()$datapath)
    }
    reconstruct_ipd(
      n_at_risk = risk_data,
      p_survival = km_data
...
output$int_pfs_ipd_source <- renderText({
  if(is.null(int_pfs_ipd_file())){
    "Currently using: Default Intervention PFS IPD dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_pfs_ipd_file()$name
    )
  }
})

output$int_pfs_survival_source <- renderText({
  if(is.null(int_pfs_km_survival_file())){
    "Currently using: Default Intervention PFS Survival dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_pfs_km_survival_file()$name
    )
  }
})

output$int_pfs_risk_source <- renderText({
  if(is.null(int_pfs_km_risk_file())){
    "Currently using: Default Intervention PFS Risk dataset"
  } else {
    paste(
      "Currently using uploaded file:",
      int_pfs_km_risk_file()$name
    )
  }
})
...
output$download_int_pfs_model_output <- downloadHandler(
  filename = function() {
    paste0("Intervention_PFS_Model_Output_", Sys.Date(), ".xlsx")
  },
  content = function(file) {
    wb <- openxlsx::createWorkbook()
    #################################################
    #### Parameters
    #################################################
    params <- extract_model_parameters(int_pfs_models())
    openxlsx::addWorksheet(wb, "Parameters")
    openxlsx::writeData(wb, "Parameters", params)
    #################################################
    #### Predictions
    #################################################
    openxlsx::addWorksheet(wb, "Predictions")
    openxlsx::writeData(wb, "Predictions", int_pfs_predictions())
    #################################################
    #### Save
    #################################################
    openxlsx::saveWorkbook(wb, file, overwrite = TRUE)
...
output$download_longterm_pfs_example <- downloadHandler(
  ...
  content = function(file) {
    write.csv(longterm_pfs_int, file, row.names = FALSE)
  }
)
...
int_os_object <- reactive({
  req(
    input$int_os_input_type %in%
      c("Median OS","Survival Data")
  )
  if(input$int_os_input_type == "Median OS"){
    req(input$int_median_os)
    validate(
      need(input$int_median_os > 0, "Median OS must be > 0")
    )
    return(
      list(
        type = "median",
...
int_occupancy_object <- reactive({
  ...
  cat("=====================\n")
  os <- tryCatch(int_os_curve(), error = function(e) e)
  pfs <- tryCatch(int_pfs_curve(), error = function(e) e)
  cat("\nOS OBJECT:\n")
  print(class(os))
  cat("\nPFS OBJECT:\n")
  print(class(pfs))
...
output$download_int_state_occupancy <- downloadHandler(
  filename = function() {
    paste0(
      "Intervention_State_Occupancy_",
...
observeEvent(input$info_longterm_validation_int, {
  showModal(
    modalDialog(
...
#########################################################
observeEvent(input$info_os_gof_int, {
  showModal(
    modalDialog(
...
#########################################################
observeEvent(input$info_pfs_gof_int, {
  showModal(
    modalDialog(
      title = "Goodness of Fit Table",
      tagList(
